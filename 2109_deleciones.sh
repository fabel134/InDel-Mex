...
di=200	#Reads falsos
pre_inicio=$(( ${inicio} - ${di} ))[ -d results ] || mkdir results

# bash Abel_deleciones.sh <input> 
# bash Abel_deleciones.sh 2824_S96.bam
# <iinput> is a bam file 
##Note
##hacer que el script agregue  guiones al bam[ -d results ] || mkdir results
...
## Producing reads list that align into fwd
awk -v ini="$inicio" -v pin="$pre_inicio" -v fini="$final" -v pfini="$pos_final" '(($2<$3)){print}' results/${fname}.blast \|sort\|uniq \> results/${fname}FWD-center ##Lista into deletion Fordward
...
#exit
## Producing reads list that align into reverse
awk -v ini="$inicio" -v pin="$pre_inicio" -v fini="$final" -v pfini="$pos_final" '(($2>$3)){print}' results/${fname}.blast \|sort\|uniq \> results/${fname}RV-center ##Lista into deletion Fordward
...
do
	name_R=$(echo $line | cut -d" " -f1)
        bi=$(echo $line | cut -d" " -f2)
        bf=$(echo $line | cut -d" " -f3)
        max_r=$(( ${inicio} < $bi  ? $bi : $inicio ))
        min_r=$(( ${final} > $bf ? $bf : $final))
        t_r=$((${min_r} - ${max_r}))
        echo ${name_R}$'\t'${t_r}
        #echo "blast" $bi $bf
        #echo "max" $max_f
