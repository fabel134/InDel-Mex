...
#foreach my $key(sort {$a <=> $b} keys %biblioteca){print "$key - > $biblioteca{$key}\n";}
#exit;
...
sub fillFile{
	...
	my $plan=shift; #planeacion langebio
	...
	foreach my $line(<FILE>){ #Here we open EpiCov File, to modify the virus names
		#print "$line\n";
		my @st=split('\t',$line);
		#Coverag [19]
		...
		$num =~ s/\R//g;#;
#		print " IMSS $IMSS_num\n"; 
#		print "numero de biblioteca ¡$num!\n";
...
#		print "$IMSS_num  -> $inum  $depth=$Depth{$inum}\n"; #From depth file original id 1-400 
...
#		print "Ensamblado numero $inum \n"; 

		if ( -e "ensamblajes/$inum-clean.fa" ){
		#	print "ensamblajes/$inum-clean.fa\n"; 
			#print "pause\n";
			#my $pause=<STDIN>;
		...
		#	my $pause=<STDIN>;
			...
			if ($ENES<900){
				...
				#print "start $start, gisaid $gisaid, $newID\n";
				$virusName="hCoV-19/Mexico/$state\_LANGEBIO_IMSS_$newID/2022";### 97% GISAID
				...
				$virusName="hCoV-19/Mexico/$state\_LANGEBIO_IMSS_$local\_NC/2022";
				...
				$counOTHER;
				my $local=substr($IMSS_num,-5);
				$virusName="hCoV-19/Mexico/$state\_LANGEBIO_IMSS_$local-OTHER/2022"; 
				$corrida++;
				}#Virus name[2]
...
sub readDepth{
	...
	foreach my $line (<FILE>){
		...
		my @st=split('\t', $line);
		#print "st[0] $st[0] ..  st[1] $st[1]\n";
		 #$st[0]=~s/(\d*)_S//;
		#my $numId=int($1);
		my $numId=$st[0];
			
		#print "$st[0].. $numId .. $st[1]\n";
		$depth{$numId}=$st[1];
	#	if($verbose){
